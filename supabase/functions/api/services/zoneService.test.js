...
import { ZoneSetError } from "../domain/zoneSet.js";
import { createZoneService } from "./zoneService.js";
...
async function harness(overrides = {}) {
  const { repository: repositoryOverrides = {}, ...serviceOverrides } =
    overrides;
  const events = [];
  ...
  let target = {
    ...
    source_geojson,
    status: "draft",
    association_approval_reference: null,
  };
  const active = {
    id: "old",
    environment: "staging",
  ...
  const repository = {
    ...
    readActiveZoneSets: () => {
      events.push("active");
      return target.status === "active" ? [target] : [active];
    },
    activateZoneSet: () => {
  ...
  const tx = (strings, ...values) => {
    events.push(
      strings.join("").includes("app.user_id")
        ? `actor:${values[0]}`
        : `role:${values[0]}`,
    );
  };
...
Deno.test("FAB-2 SERVICE: invalid create input and invalid actors never obtain a connection", async () => {
  ...
  await rejects(
    () => service.create({ actor, input: { name: "bad" } }),
    "invalid_request",
  );
  await rejects(
    () =>
  ...
  await rejects(
    () =>
      service.create({
        actor: { ...actor, profile: { ...actor.profile, active: false } },
        input: validInput,
      }),
    "forbidden",
  );
...
Deno.test("FAB-2 SERVICE: inactive profile, missing set, checksum mismatch and multiple active sets roll back", async () => {
  ...
  for (
    const [overrides, expected] of [
      ...
      [
        {
          repository: { readActiveZoneSets: () => [{ id: "a" }, { id: "b" }] },
        },
        "zone_set_conflict",
      ],
    ]
    ...
    await rejects(
      () => service.activate({ actor, zoneSetId: "new", input: activation }),
      expected,
    );
...
Deno.test("FAB-2 SERVICE: maps expected database failures without exposing unknown errors", async () => {
  ...
  assert(
    new ZoneSetError("invalid_request", "safe").code === "invalid_request",
    "Typed errors stay explicit",
  );
});
